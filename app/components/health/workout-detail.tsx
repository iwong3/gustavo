...
import { IconBarbell, IconNotes } from '@tabler/icons-react'
...
type Props = {
    workout: Workout
    /** Every workout, for the per-group cadence stats and exercise history. */
    allWorkouts: Workout[]
}
...
function getExerciseWeightHistory(
    ...
    const cutoff =
        new Date(currentWorkoutDate + 'T00:00:00').getTime() - 90 * 86400000
...
function getMuscleGroupStats(
    ...
    for (const w of allWorkouts) {
        ...
        const hasGroup = w.muscleGroups.some(
            (mg) => !isTarget(mg.name) && mg.name === groupName
        )
    ...
    const daysSinceLast = lastDate
        ? Math.round(
              (workoutTs - new Date(lastDate + 'T00:00:00').getTime()) /
                  86400000
          )
        : null
        ...
        <Box sx={sectionSx}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.25 }}>
                <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 0.5 }}>
                    <Typography
                        sx={{ fontSize: 16, fontWeight: 800, lineHeight: 1 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 0.5 }}>
                    ...
                    <Typography
                        ...
                        {stats.avgDaysBetween === 1
                            ? 'day avg between workouts'
                            : 'days avg between workouts'}
                    </Typography>
                </Box>
            </Box>
        </Box>
            ...
            <svg width={width} height={height} style={{ display: 'block' }}>
                ...
                {(() => {
                    ...
                    for (let i = 1; i < points.length; i++) {
                        if (points[i].weight >= points[maxIdx].weight)
                            maxIdx = i
                        if (points[i].weight < points[minIdx].weight) minIdx = i
                    }
                    const labelIndices = new Set(
                        maxIdx === minIdx ? [maxIdx] : [maxIdx, minIdx]
                    )
...
function ExerciseCard({
    ...
    const weightPoints = useMemo(
        () =>
            !we.exercise.isBodyweight || we.weightLbs
...
/**
 * Workout detail — the body of /gustavo/health/exercise/[id]. Content only:
 * the route page adds the PageActionBar (Delete | Duplicate | Edit) and the
 * delete dialog, and the gallery renders this on fixtures.
 */
export function WorkoutDetail({ workout, allWorkouts }: Props) {
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
            ...
            {/* ── Muscle Groups with cadence stats ───────────── */}
            <SectionDivider />
            <Box sx={{ px: 2, pb: 1 }}>
                <Typography
                    sx={{
