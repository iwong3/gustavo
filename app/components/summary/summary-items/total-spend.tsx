...
export const TotalSpend = () => {
    ...
    useEffect(() => {
        ...
        const splitters = Object.entries(splitBetweenFilter).filter(
            ([_, isActive]) => isActive
        )
        ...
        const locations = Object.entries(locationFilter).filter(
            ([_, isActive]) => isActive
        )
        ...
        const paidBys = Object.entries(paidByFilter).filter(
            ([_, isActive]) => isActive
        )
        if (paidBys.length > 0) {
            spendTitle +=
